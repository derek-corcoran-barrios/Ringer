...
four <- ggplot(data = NBA_Year_Position, aes(x = Year, y = pct)) + geom_point(aes(color = Position)) + geom_smooth(aes(color=Position))

NBA<- filter(NBA, Position != "NA")
NBA<- filter(NBA, Three_pct != "NaN")

Five <- ggplot(data = NBA, aes(x = Year, y = Three_pct)) + geom_smooth(aes(color=Position))
```

```{r, echo=FALSE, cache=TRUE, fig.cap= "Three point percentage trends from 1998 to 2016 by position"}
ggthemr("earth", type="outer", layout="scientific", spacing=2)

ggplot(data = NBA, aes(x = Year, y = Three_pct)) + stat_smooth(method="lm", aes(weight=TotalMinutesPlayed, color = Position)) + ylab("3-Point Shooting %")
...
rank <- filter(rank, Position != "All")
ggplot(rank, aes(x = Year, y = pct)) + geom_point(aes(colour = Position)) + geom_smooth(aes(color=Position))
```